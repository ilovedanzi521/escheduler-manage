...
public class ProcessInstanceController extends BaseController {
    ...
    /**
     * @Title: querySubProcessInstanceByTaskId
     * @Description 获取子节点信息
     * @param loginUser
     * @param projectName
     * @param processId
     * @return cn.escheduler.api.utils.Result
     * @throws
     * @author wanglei
     * @Date 2019/8/12/15:23
     */
    @GetMapping(value = "/select-sub-task")
    @ResponseStatus(HttpStatus.OK)
    public Result querySubProcess(@ApiIgnore @RequestAttribute(value = Constants.SESSION_USER) User loginUser,
                                  @ApiParam(name = "projectName", value = "PROJECT_NAME", required = true) @PathVariable String projectName,
                                  @RequestParam("processId") Integer processId) {
        try {
            Map<String, Object> result = processInstanceService.querySubProcess(loginUser, projectName, processId);
            return returnDataList(result);
        } catch (Exception e) {
            logger.error(QUERY_SUB_PROCESS_INSTANCE_DETAIL_INFO_BY_TASK_ID_ERROR.getMsg(), e);
            return error(Status.QUERY_SUB_PROCESS_INSTANCE_DETAIL_INFO_BY_TASK_ID_ERROR.getCode(), Status.QUERY_SUB_PROCESS_INSTANCE_DETAIL_INFO_BY_TASK_ID_ERROR.getMsg());
