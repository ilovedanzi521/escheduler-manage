...
public class TaskInstanceMapperProvider {
    ...
    public String countTaskInstanceStateByUser(Map<String, Object> parameter) {
        return new SQL() {
            {
                SELECT("t.state as state, count(0) as count");
                FROM(TABLE_NAME + " t");
     ...
     */
    public String queryTaskSubNode(Map<String, Object> parameter){
        StringBuffer instanceIdsStr = new StringBuffer();
        Object[] instanceIdArray = (Object[]) parameter.get("processInstanceIds");
        for (int i = 0; i < instanceIdArray.length; i++) {
            instanceIdsStr.append(instanceIdArray[i]);
            if (i < instanceIdArray.length - 1) {
                instanceIdsStr.append(",");
            }
        }
        return new SQL() {
            {
                ...
                WHERE("instance.process_instance_id in (" + instanceIdsStr.toString() + ")");
                ORDER_BY("instance.start_time asc ");
            }
        }.toString();
    }
    /**
