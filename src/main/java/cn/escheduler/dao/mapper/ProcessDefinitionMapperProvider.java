...
public class ProcessDefinitionMapperProvider {
        ...
        return new SQL() {
            {
                ...
                //展现形式
                VALUES("`layout`",  EnumFieldUtil.genFieldStr("processDefinition.layout", LayoutType.class));
            }
        }.toString();
    }
    ...
    public String queryByDefineId(Map<String, Object> parameter) {
        return new SQL() {
            {
                SELECT("td.id,td.name,td.version,td.release_state,td.project_id,td.user_id,td.process_definition_json,td.desc,td.global_params,td.flag,td.locations,td.connects,td.receivers,td.receivers_cc,td.create_time,td.timeout,td.update_time,u.user_name,p.name as project_name");

                FROM(TABLE_NAME + " td");
                JOIN("t_escheduler_user u ON td.user_id = u.id");
                JOIN("t_escheduler_project p ON td.project_id = p.id");
                WHERE("td.id = #{processDefinitionId}");
            }
        }.toString();
    }
    ...
    public String queryByDefineName(Map<String, Object> parameter) {
        return new SQL() {
            {
                SELECT("td.id,td.name,td.version,td.release_state,td.project_id,td.user_id,td.process_definition_json,td.desc,td.global_params,td.flag,td.locations,td.connects,td.receivers,td.receivers_cc,td.create_time,td.timeout,td.update_time,u.user_name,p.name as project_name,t.tenant_code,t.tenant_name,q.queue,q.queue_name");
                ...
                WHERE("td.name = #{processDefinitionName}");
            }
        }.toString();
    }
    ...
    public String queryDefineListPaging(Map<String, Object> parameter) {
        return new SQL() {{
            SELECT("td.id,td.name,td.version,td.release_state,td.project_id,td.user_id,td.process_definition_json,td.desc,td.global_params,td.flag,td.locations,td.connects,td.receivers,td.receivers_cc,td.create_time,td.timeout,td.update_time,sc.schedule_release_state");
            FROM(TABLE_NAME + " td");
    ...
    public String queryDefinitionListByIdList(Map<String, Object> parameter) {
        ...
        return new SQL() {{
            SELECT("id,name,version,release_state,project_id,user_id,process_definition_json,`desc`,global_params,flag,locations,connects,receivers,receivers_cc,create_time,timeout,update_time");
