  ...
  <div class="x-date-packer-time" :style="{width: type==='daterange' ? '305px' : 'auto'}">
    <div class="packer-time-body" :class="[getType()]">
      ...
      <div class="picker-time-div time-hours" ref="hours">
        <ul>
          <li v-for="(item,index) in 24" :class="{'time-act': index == hours}" @click="change(index, 'hours')">
            {{numLen(index)}}
          </li>
        </ul>
      </div>
      <div class="picker-time-div time-minute" ref="minute">
        <ul>
          <li v-for="(item,index) in 60" :class="{'time-act': index == minute}" @click="change(index, 'minute')">
            {{numLen(index)}}
          </li>
        </ul>
      </div>
      <div class="picker-time-div time-second" ref="second">
        <ul>
          <li v-for="(item,index) in 60" :class="{'time-act': index == second}" @click="change(index, 'second')">
            {{numLen(index)}}
          </li>
        </ul>
      </div>
    ...
    <div class="packer-time-body" :class="[getType()]" v-if="type==='daterange'" style="float:right">
      ...
      <div class="picker-time-div time-hours bd-left" ref="hours1">
        <ul>
          <li v-for="(item,index) in 24" :class="{'time-act': index == hours1}" @click="changeLast(index, 'hours1')">
            {{numLen(index)}}
          </li>
        </ul>
      </div>
      <div class="picker-time-div time-minute" ref="minute1">
        <ul>
          <li v-for="(item,index) in 60" :class="{'time-act': index == minute1}" @click="changeLast(index, 'minute1')">
            {{numLen(index)}}
          </li>
        </ul>
      </div>
      <div class="picker-time-div time-second" ref="second1">
        <ul>
          <li v-for="(item,index) in 60" :class="{'time-act': index == second1}" @click="changeLast(index, 'second1')">
            {{numLen(index)}}
          </li>
        </ul>
      </div>
