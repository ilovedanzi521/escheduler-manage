<template>
  <div class="date-packer-daterange-body">
    <div class="date-packer-daterange">
      <div class="date-packer-opt" v-if="options" :style="{width: optionsWidth + 'px'}">
        <div class="picker-panel-shortcut" :style="item.style"
             ...
             :class="{act: getOptItemAct(item)}">{{item.text}}
        </div>
      </div>
      <div class="date-interval" ref="datePackerDay"
           :style="{width: 202 * panelNum + 'px', marginLeft: options ? optionsWidth + 'px' : ''}">
    ...
    <confirm v-if="isConfirm()" :format="format" @confirm-btn="$emit('confirm')" @cancel="cancel" type="daterange"
             ref="confirm" @time-change="timeChange">
