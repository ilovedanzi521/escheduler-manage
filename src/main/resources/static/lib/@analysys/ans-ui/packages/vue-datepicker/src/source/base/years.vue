<template>
  <div class="x-date-packer-ym">
    <div class="x-date-packer-hd">
      <span class="hd-icon hd-icon-left" style="display:block"><i class="ans-icon-arrow-left"
                                                                  @click="switchYear('left')"></i></span>
      <span class="hd-txt" @click="selectText">{{year + t('ans.datepicker.year')}}</span>
      <span class="hd-icon hd-icon-right" style="display:block"><i class="ans-icon-arrow-right"
                                                                   @click="switchYear('right')"></i></span>
    </div>
    <div class="x-date-packer-item">
      <div class="x-date-packer-year" v-if="type === 'y'">
        <div class="year-item" v-for="item in yearList"><span :class="{act: isYear(item)}" @click="selectYear(item)">{{item}}</span>
        </div>
      </div>
      <div class="x-date-packer-month" v-else>
        <div class="year-item" v-for="item in 12"><span :class="{act: isMonth(item)}" @click="selectMonth(item)">{{t('ans.datepicker.month' + item)}}</span>
        </div>
      </div>
    </div>
  </div>
