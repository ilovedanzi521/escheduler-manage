<template>
  <div class="x-date-packer-panel" ref="dateDay">
    ...
    <div class="x-date-packer-hd">
      <span class="hd-icon hd-icon-left"><i class="ans-icon-arrow-to-left" @click="setToDate('subtract', 'y')"></i> <i
        class="ans-icon-arrow-left ans-icon-arrow-left" @click="setToDate('subtract', 'M')"></i></span>
      ...
      <span class="hd-icon hd-icon-right"><i class="ans-icon-arrow-right" @click="setToDate('add', 'M')"></i> <i
        class="ans-icon-arrow-to-right" @click="setToDate('add', 'y')"></i></span>
    </div>
    <div class="x-date-packer-day">
      ...
      <div class="x-date-packer-day-day">
        <label class="lattice" v-for="i in weeks"></label><span class="lattice em" v-for="d in days"
                                                                v-attr="hoverDate(d)" @click="selected(d, $event)"
                                                                :key="uuId()" @mouseover="hover(d, $event)"
                                                                :class="{'picker-disabled': getDisabledDate(d)}">
          <label :data-mouseover="year + '-' + month + '-' + d" class="dataMouseoverAct"><em :data-today="today(d)"
                                                                                             :data-select="selectDay(d)">{{d}}</em></label>
        </span>
      </div>
    </div>
  </div>
