<template>
  <x-poptip
    ...
    <div slot="reference">
      <slot name="input" :placeholder="placeholder" :value="text" :suffix-icon="suffixIcon" :prefix-icon="prefixIcon"
            :disabled="disabled" :size="size" :readonly="readonly">
        <x-input :value="text" :clearable="clearable" :placeholder="placeholder" :suffix-icon="suffixIcon"
                 :prefix-icon="prefixIcon" :size="size" :readonly="readonly" @on-clear="empty"></x-input>
      </slot>
    </div>
        ...
        if (data && data.length) {
          ...
          if (type === 'daterange') {
            if (data.length === 1) {
              ...
              let text = []
              fmtDate.forEach(o => {
                text.push(o.join(this.rangeSeparator))
              })
              this.text = text
            }
          }
